...
class PartsResort():
    ...
    def update(self, points, order):
        ...
        for i in range(self.num_center):  # 4
    ...
    def classify(self, points, is_train):
        ...
        if np.sum(self.count) == 0:
            ...
            order = np.zeros([batch, topN], dtype=int)
            for i in range(points.shape[0]):
                topn_points = points[i]
                order[i] = self.graph_assign(topn_points)
        if is_train:
            self.update(points, order)  # points(16,4,1024),order(16,4)
        return order
...
#     # PC.classify(points)
#
#     PC = PartsResort(5, 3072)
#
#     points = np.random.randint(0, 10, size=[2, 5, 3072])
#     PC.classify(points,1)
#     points = np.random.randint(0, 10, size=[2, 5, 3072])
#     PC.classify(points,1)