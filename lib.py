...
def save_mapsone(X: torch.Tensor, maps: torch.Tensor, epoch: int, model_name: str, device: torch.device,global_index:int) -> None:
    X = X.squeeze(0)  # 去掉第一个维度
    maps = maps.squeeze(0)  # 去掉第一个维度
    # Create meshgrid for map dimensions
    grid_x, grid_y = torch.meshgrid(torch.arange(maps.shape[1]), torch.arange(maps.shape[2]))
    grid_x = grid_x.unsqueeze(0).to(device)
    grid_y = grid_y.unsqueeze(0).to(device)

    # Calculate attention map centroids
    map_sums = maps.sum(2).sum(1).detach()
    maps_x = grid_x * maps
    maps_y = grid_y * maps
    loc_x = maps_x.sum(2).sum(1) / map_sums
    loc_y = maps_y.sum(2).sum(1) / map_sums

    # Prepare visualization
    fig, ax = plt.subplots(figsize=(6, 6))


    # Resize and overlay the image and attention maps
    landmarks = landmarks_to_rgb(maps[:-1, :, :].detach().cpu().numpy())
    overlay = (
        skimage.transform.resize(landmarks, (256, 256)) +
        skimage.transform.resize(X.permute(1, 2, 0).cpu().numpy(), (256, 256))
    )
    ax.imshow(overlay)

    # Plot landmarks
    x_coords = loc_y[:-1].detach().cpu() * 256 / maps.shape[-1]
    y_coords = loc_x[:-1].detach().cpu() * 256 / maps.shape[-2]
    cols = COLORS[:loc_x.shape[0] - 1]
    for xi, yi, col_i, mark in zip(x_coords, y_coords, cols, range(len(x_coords))):
        ax.scatter(xi, yi, color=col_i, marker=f'${mark}$')

    # Save the result
    os.makedirs(f'./results_{model_name}', exist_ok=True)
    plt.savefig(f'./results_{model_name}/{epoch}_{global_index}')
    plt.close()
