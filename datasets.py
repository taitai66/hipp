...
class PartImageNetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            path = f"{self.data_path}/train/{folder}/{imgname}"
        elif self.mode == 'test':
            path = f"{self.data_path}/train/{folder}/{imgname}"
...
class AIR():
    def __init__(self, root, is_train=True, image_size: int = 224, data_len=None):
        self.root = root
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            img = transforms.Resize((225, 225))(img)
            img = transforms.RandomCrop(self.image_size, padding=8)(img)
            img = transforms.RandomHorizontalFlip()(img)
...
class NXBirdsataset(torch.utils.data.Dataset):
    def __init__(self, data_path: str, split: float = 1, mode: str = 'train', train_samples: list = None,
                 image_size: int = 448, evaluate: bool = False):
        """
        CUB dataset
        Parameters
        ----------
        ...
        self.mode = mode
        self.image_size = image_size
        train_test = pd.read_csv(os.path.join(data_path, 'labels/train_test_split.txt'), delim_whitespace=True, names=['id', 'train'])
        image_names = pd.read_csv(os.path.join(data_path, 'labels/images.txt'), delim_whitespace = True, names=['id', 'filename'])
        labels = pd.read_csv(os.path.join(data_path, 'labels/image_class_labels.txt'), delim_whitespace=True, names=['id', 'label'])
        image_parts = pd.read_csv(os.path.join(data_path, 'labels/parts/part_locs.txt'), delim_whitespace=True, names=['id', 'part_id', 'x', 'y', 'visible'])
        dataset = train_test.merge(image_names, on='id')
        dataset = dataset.merge(labels, on='id')
...
class DF20M(torch.utils.data.Dataset):
    def __init__(self, csv_file, image_dir, transform=None):
        """
        Args:
            csv_file (str): CSV 文件路径（例如 train.csv）。
            image_dir (str): 图片所在文件夹路径。
            transform (callable, optional): 数据增强或预处理的 transform。
        """
        self.data_frame = pd.read_csv(csv_file)
        self.image_dir = image_dir
        self.transform = transform
        # 获取所有类别名称并创建类别到索引的映射
        self.class_to_idx = {cls_name: idx for idx, cls_name in enumerate(self.data_frame['specificEpithet'].unique())}
    ...
    def __getitem__(self, idx):
        # 获取图片路径和类别名称
        # print(self.data_frame.head())  # 打印前几行
        # print(self.data_frame.dtypes)  # 检查每列的数据类型
        img_name = os.path.join(self.image_dir, self.data_frame.loc[idx, 'image_path'])
        label_name = self.data_frame.loc[idx, 'specificEpithet']
        # 将标签名称映射到类别索引
