...
from nets_visual import IndividualLandmarkNet
...
from train_visual import train, validation
...
def main():
    ...
    parser.add_argument('--model_name', help='Name under which the model will be saved',
                        default='wildfish73_visual2048')  # , required=True
    parser.add_argument('--data_root',
                        help='directory that contains the celeba, cub, or partimagenet folder',
                        default='datasets')  # , required=True
    parser.add_argument('--dataset', help='The dataset to use. Choose NXbirds, butterfly200_73, snake135 or wildfish_73.',
    ...
    parser.add_argument('--save_figures', default=True,
    ...
    if args.dataset == 'NXbirds':
        ...
        num_cls = 206
    elif args.dataset == 'butterfly200_73':
        #part
        train_transforms = transforms.Compose([
            transforms.Resize(size=224, antialias=True),  # 调整图像大小
    ...
    if (args.dataset.lower() == 'butterfly200_73' or args.dataset.lower() == 'nxbirds'or args.dataset.lower() == 'snake135'
            or args.dataset.lower() == 'wildfish_73' or args.dataset.lower() == 'cub100' or args.dataset.lower() == 'fish_73' ):
        ...
        if not args.only_test:
            ...
            if all_losses:
                net, all_losses = train(net, optimizer, train_loader, device, epoch, 0, loss_fn,
                                        loss_hyperparams, writer, all_losses)#visual
                # net, all_losses = train(net, optimizer, train_loader, device, epoch, 0, loss_fn,
                ...
                net, all_losses = train(net, optimizer, train_loader, device, epoch, epoch_leftoff,
                                                        loss_fn, loss_hyperparams, writer)#visual
                # net, all_losses = train(net, optimizer, train_loader, device, epoch, epoch_leftoff,
            ...
            fixed_indices =  [100, 210, 301, 400, 502, 603, 704, 800,900,1010]  # 需要可视化的固定图片索引
            validation(device, net, val_loader, epoch, args.model_name, args.save_figures, writer,fixed_indices)#visual
